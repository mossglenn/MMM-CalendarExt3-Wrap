...
const popoverSupported = Object.prototype.hasOwnProperty.call(
...
if (!popoverSupported)
  console.info("This browser doesn't support popover yet. Update your system.");
const animationSupported =
  typeof window !== 'undefined' && window?.mmVersion
    ? +window.mmVersion.split('.').join('') >= 2250
    : false;

Module.register('MMM-CalendarExt3-Wrap', {
  defaults: {
  ...
  getStyles() {
    let css = ['MMM-CalendarExt3-Wrap.css'];
    return css;
  },
  ...
  getMoment(options) {
    let moment = options.referenceDate
    ...
    switch (options.mode) {
      case 'day':
        moment = new Date(
          moment.getFullYear(),
          moment.getMonth(),
          moment.getDate() + options.dayIndex,
        );
        break;
      case 'month':
        moment = new Date(
          moment.getFullYear(),
          moment.getMonth() + options.monthIndex,
          1,
        );
        break;
      case 'week':
      default:
        moment = new Date(
          moment.getFullYear(),
          moment.getMonth(),
          moment.getDate() + 7 * options.weekIndex,
        );
    }
    return moment;
  },
  ...
  regularizeConfig(options) {
    ...
    options.locale =
      Intl.getCanonicalLocales(
        options.locale ?? config?.locale ?? config?.language,
      )?.[0] ?? '';
    ...
    options.firstDayOfWeek =
      options.firstDayOfWeek !== null
        ? options.firstDayOfWeek
        : weekInfo.firstDay ?? weekInfoFallback.firstDay;
    options.minimalDaysOfNewYear =
      options.minimalDaysOfNewYear !== null
        ? options.minimalDaysOfNewYear
        : weekInfo.minimalDays ?? weekInfoFallback.minimalDays;
    options.weekends = (
      Array.isArray(options.weekends) && options.weekends?.length
        ? options.weekends
        : weekInfo.weekend ?? []
    ).map((d) => d % 7);
    ...
    this.notifications = {
      weatherNotification:
        options.weatherNotification ??
        this.defaulNotifications.weatherNotification,
      weatherPayload:
        typeof options.weatherPayload === 'function'
          ? options.weatherPayload
          : this.defaulNotifications.weatherPayload,
      eventNotification:
        options.eventNotification ?? this.defaulNotifications.eventNotification,
      eventPayload:
        typeof options.eventPayload === 'function'
          ? options.eventPayload
          : this.defaulNotifications.eventPayload,
    };

    options.mode = ['day', 'month', 'week'].includes(options.mode)
      ? options.mode
      : 'week';
  ...
  start() {
    ...
    let _moduleLoaded = new Promise((resolve, reject) => {
      import(`/${this.file('CX3_Shared/CX3_shared.mjs')}`)
        .then((m) => {
          this.library = m;
  ...
  preparePopover() {
    ...
    fetch(this.file(this.config.popoverTemplate))
      .then((response) => {
      ...
      .then((text) => {
        const template = new DOMParser()
          ...
          .querySelector('#CX3_T_POPOVER')
          .content.querySelector('.popover');
  ...
  dayPopover(cDom, events, options) {
    ...
    container.innerHTML = '';
    const ht = popover
      .querySelector('template#CX3_T_EVENTLIST')
      .content.cloneNode(true);
    container.append(document.importNode(ht, true));
    let header = container.querySelector('.header');
    header.innerHTML = new Intl.DateTimeFormat(options.locale, {
      dateStyle: 'full',
    })
      .formatToParts(new Date(+cDom.dataset.date))
    ...
    events.forEach((e) => {
      const pOption = e.fullDayEvent
        ? { dateStyle: 'short' }
        : { dateStyle: 'short', timeStyle: 'short' };

      const item = popover
        .querySelector('template#CX3_T_EVENTITEM')
      ...
      time.innerHTML = new Intl.DateTimeFormat(options.locale, pOption)
        .formatRangeToParts(new Date(+e.startDate), new Date(+e.endDate))
        .reduce((prev, cur, curIndex) => {
          const result = `${prev}<span class="eventTimeParts ${cur.type} seq_${curIndex} ${cur.source}">${cur.value}</span>`;
          return result;
        }, '');
  ...
  eventPopover(eDom, options) {
    ...
    container.innerHTML = '';
    const ht = popover
      .querySelector('template#CX3_T_EVENTDETAIL')
      .content.cloneNode(true);
    container.append(document.importNode(ht, true));
    ...
    header.style.setProperty(
      '--calendarColor',
      eDom.style.getPropertyValue('--calendarColor'),
    );
    header.style.setProperty(
      '--oppositeColor',
      eDom.style.getPropertyValue('--oppositeColor'),
    );
    ...
    ps.forEach((c) => {
      if (eDom.dataset[c]) {
        const ct = popover
          .querySelector('template#CX3_T_CRITERIA')
          .content.firstElementChild.cloneNode(true);
    ...
    const pOption =
      eDom.dataset.fullDayEvent === 'true'
        ? { dateStyle: 'short' }
        : { dateStyle: 'short', timeStyle: 'short' };
    n.querySelector('.value').innerHTML = new Intl.DateTimeFormat(
      options.locale,
      pOption,
    )
  ...
  notificationReceived(notification, payload, sender) {
    ...
    if (notification === this.notifications.eventNotification) {
      ...
      this.eventPool.set(
        sender.identifier,
        JSON.parse(JSON.stringify(convertedPayload)),
      );
    }
      ...
      const callback = (mutationsList) => {
        for (let mutation of mutationsList) {
          const content = document.querySelector(
            `#${this.identifier} .module-content .CX3`,
          );
          if (mutation.addedNodes.length > 0)
            this.updated(content, this.activeConfig);
        }
      };
    ...
    if (notification === this.notifications.weatherNotification) {
      ...
      if (
        ...
        ((this.activeConfig.weatherLocationName &&
          convertedPayload.locationName.includes(
            this.activeConfig.weatherLocationName,
          )) ||
          !this.activeConfig.weatherLocationName) &&
        Array.isArray(convertedPayload?.forecastArray) &&
        convertedPayload?.forecastArray.length
      ) {
        ...
        if (
          ...
          Log.warn(
            `"weatherLocationName: '${this.activeConfig.weatherLocationName}'" doesn't match with location of weather module ('${convertedPayload.locationName}')`,
          );
        }
      }
    }
    ...
    if (
      ['CX3_GLANCE_CALENDAR', 'CX3_MOVE_CALENDAR', 'CX3_SET_DATE'].includes(
        notification,
      )
    ) {
      console.warn(
        "[DEPRECATED]'CX3_GLANCE_CALENDAR' notification was deprecated. Use 'CX3_SET_CONFIG' instead. (README.md)",
      );
    }
    if (
      payload?.instanceId &&
      payload?.instanceId !== this.activeConfig?.instanceId
    )
      return;
  ...
  getDom() {
    ...
    this.sendNotification('CX3_DOM_UPDATED', {
  ...
  async draw(dom, options) {
    ...
    const startDayOfWeek = getBeginOfWeek(
      new Date(Date.now()),
      options,
    ).getDay();
    ...
    dom.style.setProperty(
      '--title-max-lines',
      String(options.titleMaxLines || 1),
    );
    dom.dataset.debugWrap = options.debugWrapLayout ? 'true' : 'false';
    dom.style.setProperty(
      '--displayEndTime',
      options.displayEndTime ? 'inherit' : 'none',
    );
    dom.style.setProperty(
      '--displayWeatherTemp',
      options.displayWeatherTemp ? 'inline-block' : 'none',
    );
    ...
    const makeCellDom = (d) => {
      ...
      options.weekends.forEach((w, i) => {
        if (tm.getDay() === w)
          cell.classList.add('weekend', `weekend_${i + 1}`);
      });
      ...
      let dParts = new Intl.DateTimeFormat(
        options.locale,
        options.cellDateOptions,
      ).formatToParts(tm);
    ...
    const rangeCalendar = (moment, options) => {
      ...
      switch (options.mode) {
        ...
        case 'month':
          boc = getBeginOfWeek(
            new Date(moment.getFullYear(), moment.getMonth(), 1),
            options,
          );
          eoc = getEndOfWeek(
            new Date(moment.getFullYear(), moment.getMonth() + 1, 0),
            options,
          );
          break;
        case 'week':
        default:
          ...
          eoc = getEndOfWeek(
            new Date(
              boc.getFullYear(),
              boc.getMonth(),
              boc.getDate() + 7 * (options.weeksInView - 1),
            ),
            options,
          );
          break;
    ...
    const makeDayHeaderDom = (dom, options, range) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        dDom.innerHTML = new Intl.DateTimeFormat(
          options.locale,
          options.headerWeekDayOptions,
        ).format(dm);
        dayDom.append(dDom);
      }
    ...
    // Helper: Calculate row span for a title element given actual column width
    const calculateRowSpan = (event, columnWidth, options) => {
      if (!options.titleMaxLines || options.titleMaxLines <= 1) return 1;
      const rowPx = parseFloat(options.eventHeight) || 22;
      const tempEvent = renderEventAgenda(event, options, moment);
      const titleEl = tempEvent.querySelector('.title');
      if (!titleEl) return 1;
      titleEl.classList.add('cx3-wrap-title');
      titleEl.style.setProperty(
        '--title-max-lines',
        String(options.titleMaxLines),
      );
      const measureContainer = document.createElement('div');
      measureContainer.style.cssText = `position:absolute; visibility:hidden; width:${columnWidth}px; left:-9999px; top:-9999px;`;
      measureContainer.append(tempEvent);
      dom.append(measureContainer);
      const usedRows = Math.max(
        1,
        Math.min(
          options.titleMaxLines,
          Math.ceil(titleEl.scrollHeight / rowPx),
        ),
      );
    ...
    // Helper: Get actual column width from rendered grid
    const getColumnWidth = (gridElement) => {
      const firstCell = gridElement.querySelector('.cell');
      if (!firstCell) return 200;
    ...
    const makeWeekGridDom = (dom, options, events, { boc, eoc }) => {
      const getMaxEventLines = ({ maxEventLines }, weekCount) => {
        ...
        if (typeof maxEventLines === 'object') {
          if (!Array.isArray(maxEventLines)) {
            ...
            lines = [
              ...maxEventLines.filter(
                (v) => Number.isInteger(+v) && Number.isFinite(+v),
              ),
            ];
          }
        }
      ...
      do {
        ...
        for (let i = 0; i < 7; i++) {
          if (i)
            cm = new Date(cm.getFullYear(), cm.getMonth(), cm.getDate() + 1);
        ...
        for (let event of eventsOfWeek) {
          ...
          if (popoverSupported) {
            if (!eDom.id)
              eDom.id = `${eDom.dataset.calendarSeq}_${
                eDom.dataset.startDate
              }_${eDom.dataset.endDate}_${new Date(Date.now()).getTime()}`;
        ...
        // Calculate row spans for multi-line titles after rendering
        if (options.titleMaxLines > 1) {
          const columnWidth = getColumnWidth(ccDom);
          eventsOfWeek.forEach((event) => {
            if (!event.skip) {
              const rowSpan = calculateRowSpan(event, columnWidth, options);
              rowSpanMap.set(event, rowSpan);
            }
          });
        }
        ...
        for (let i = 0; i < dateCells.length; i++) {
          ...
          let thatDayEvents = eventsOfWeek.filter((ev) => {
            return !(
              ev.endDate <= dateStart.valueOf() ||
              ev.startDate > dateEnd.valueOf()
            );
          });
          dateCell.dataset.events = thatDayEvents.length;
          dateCell.dataset.hasEvents =
            thatDayEvents.length > 0 ? 'true' : 'false';
          ...
          if (options.showMore) {
            if (options.titleMaxLines > 1) {
              let rowsUsed = 0;
              thatDayEvents.forEach((ev) => {
                if (!ev.skip) rowsUsed += rowSpanMap.get(ev) || 1;
              });
              if (rowsUsed > maxEventLines) {
                dateCell.classList.add('hasMore');
                dateCell.style.setProperty('--more', rowsUsed - maxEventLines);
              }
            } else {
              const skipped = thatDayEvents.filter((ev) => ev.skip).length;
              const noskip = thatDayEvents.length - skipped;
              const noskipButOverflowed =
                noskip > maxEventLines ? noskip - maxEventLines : 0;
          ...
          if (popoverSupported) {
            if (!dateCell.id)
              dateCell.id = `${dateCell.dataset.date}_${new Date(
                Date.now(),
              ).getTime()}`;
  ...
  getHeader() {
    if (this.data.header && this.data.header.trim() !== '')
    ...
    if (!this.activeConfig.customHeader && this.activeConfig.mode === 'month') {
      ...
      return new Intl.DateTimeFormat(locale, titleOptions).format(
